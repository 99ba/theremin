...
TRAUMEREI_MELODY = """
5,:2 1:5 7,:1 1:1 3:1 5:1 1':5 7:1 6:1 |
5:1 1':1 2:1 3:1 4:1 6:1 1:1 2:1 3:1 5:1 2:4 |
5,:2 1:5 7,:1 1:1 3:1 5:1 3':4 2':1 1':1 6:1 |
5:1 1':1 2:1 3:1 4:1 6:1 2:1 3:1 4:1 6:1 6,:1 7,:1 1:4
"""

CANGHAI_MELODY = """
6:0.75 5:0.25 3:0.5 2:0.5 1:2 |
3:0.75 2:0.25 1:0.5 6,:0.5 5,:2 |
5,:0.75 6,:0.25 5,:0.75 6,:0.25 1:0.75 2:0.25 3:0.5 5:0.5 |
6:0.75 5:0.25 3:0.25 2:0.25 1:0.5 2:2 |
6:0.5 6:0.25 5:0.25 3:0.5 2:0.5 1:2 |
3:0.75 2:0.25 1:0.5 6,:0.5 5,:2 |
5,:0.75 6,:0.25 5,:0.75 6,:0.25 1:0.75 2:0.25 3:0.5 5:0.5 |
6:0.75 5:0.25 3:0.25 2:0.25 1:0.5 2:2 |
6:0.5 6:0.25 5:0.25 3:0.5 2:0.5 1:2 |
...
SONGBIE_MELODY = """
5 3:0.5 5:0.5 1':1.5 7:0.5 |
6 1' 5:2 |
5 2:0.5 3:0.5 4:1.5 7,:0.5 |
1:4 |
5 3:0.5 5:0.5 1':2 |
6 1':0.5 6:0.5 5:2 |
5 1:0.5 2:0.5 3 2:0.5 1:0.5 |
2:3 0 |
5 3:0.5 5:0.5 1':1.5 7:0.5 |
6 1' 5:2 |
5 2:0.5 3:0.5 4:1.5 7,:0.5 |
...
6 1' 1':2 |
7 6:0.5 7:0.5 1':2 |
6:0.5 7:0.5 1':0.5 6:0.5 6:0.5 5:0.5 3:0.5 1:0.5 |
2:3 0 |
5 3:0.5 5:0.5 1':1.5 7:0.5 |
6 1' 5:2 |
5 2:0.5 3:0.5 4:1.5 7,:0.5 |
...
def split_guide_token_duration(raw_token: str, base_beats: float) -> tuple[str, float]:
    token = raw_token.strip()
    beats = base_beats
    if ":" in token:
        token, duration_units = token.rsplit(":", 1)
        beats *= max(float(duration_units), 0.0)
    elif token.endswith("."):
        token = token[:-1]
        beats *= 1.5
    return token, beats
...
def guide_token_to_midi(song: "GuideSong", token: str) -> int:
    octave_shift = 0
    ...
    return song.root_midi + MAJOR_SCALE_OFFSETS[token] + accidental + 12 * octave_shift
...
BUILTIN_GUIDE_LIBRARY = (
    ...
    GuideSong(
        ...
        scale_type="major",
        root_midi=65,
        melody=TRAUMEREI_MELODY,
        base_beats=0.5,
        guide_bpm=24.0,
    ),
    GuideSong(
        key="canghai",
        title="Cang Hai Yi Sheng Xiao",
        label="Canghai",
        root_note="A",
        scale_type="major",
        root_midi=69,
        melody=CANGHAI_MELODY,
        base_beats=1.0,
        guide_bpm=96.0,
    ),
    GuideSong(
...
def _load_external_guide_songs() -> tuple[GuideSong, ...]:
    scores_dir = Path(__file__).resolve().parents[1] / "scores"
    if not scores_dir.exists():
        return ()

    songs: list[GuideSong] = []
    for path in sorted(scores_dir.glob("*.json")):
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            continue
        if data.get("format") != "mixure_guide_song_v1" or data.get("schema"):
            continue
        try:
            songs.append(
                GuideSong(
                    key=str(data["key"]),
                    title=str(data.get("title") or data["key"]),
                    label=str(data.get("label") or data.get("title") or data["key"]),
                    root_note=str(data.get("root_note") or "C"),
                    scale_type=str(data.get("scale_type") or "major"),
                    root_midi=int(data.get("root_midi", 60)),
                    melody=str(data["melody"]),
                    base_beats=float(data.get("base_beats", 1.0)),
                    guide_bpm=float(data.get("guide_bpm", 70.0)),
                )
            )
...
GUIDE_LIBRARY = (*BUILTIN_GUIDE_LIBRARY, *_load_external_guide_songs())
...
def build_guide_midis(song: GuideSong) -> list[int]:
    ...
    for raw_token in song.melody.split():
        token, _beats = split_guide_token_duration(raw_token, song.base_beats)
        if not token or token in {"|", "-", "0"}:
            continue
        midis.append(guide_token_to_midi(song, token))
    return midis
...
class PerformanceGuide:
    ...
    def _build_events(self) -> list[GuideEvent]:
        ...
        for raw_token in self.song.melody.split():
            ...
            token, beats = split_guide_token_duration(raw_token, self.song.base_beats)
    ...
    def _token_to_midi(self, token: str) -> int:
        return guide_token_to_midi(self.song, token)
    ...
    def update(self, now: float) -> dict | None:
        ...
        alpha = clamp(self.config.GUIDE_DISPLAY_SMOOTH_ALPHA, 0.01, 1.0)
        ...
        if self.smoothed_current_point is not None:
            self.trail.append((self.smoothed_current_point, now))

        cutoff = now - self.config.GUIDE_TRAIL_SECONDS
        self.trail = [(point, ts) for point, ts in self.trail if ts >= cutoff]
        trail_points = []
        trail_span = max(self.config.GUIDE_TRAIL_SECONDS, 1e-6)
        for point, ts in self.trail:
            age_ratio = clamp((now - ts) / trail_span, 0.0, 1.0)
            trail_points.append({"point": point, "age_ratio": age_ratio})
